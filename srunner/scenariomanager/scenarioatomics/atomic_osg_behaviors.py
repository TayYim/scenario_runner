...
import py_trees
from srunner.tools.scenario_helper import transform_world_vector_to_local, adaptive_savgol_filter, adaptive_gradient
import os
import csv
import datetime
...
class OASDataCollector(AtomicBehavior):
    ...
    def update(self):
        ...
        self._data_dict["roll"].append(
            round(float(transform.rotation.roll), 2))
        self._data_dict["pitch"].append(
            round(float(transform.rotation.pitch), 2))
        ...
        velocity_local = transform_world_vector_to_local(
            transform, velocity_vector)
        ...
        self._data_dict["velocity"].append(
            round(float(velocity_local.length()), 2))
        ...
        self._data_dict["throttle"].append(
            round(float(actor_control.throttle), 2))
        ...
        # name
        self._data_dict["name"].append(self.name)
    ...
    def get_final_data(self):
        ...
        for direction in direction_list:
            ...
            # 处理加速度
            acc_data = adaptive_gradient(
                vel_data, self._data_dict["game_time"])
            ...
            # 处理加速度变化率
            jerk_data = adaptive_gradient(
                acc_data, self._data_dict["game_time"])
    ...
    def _save_result_to_csv(self):
        ...
        save data to csv
        """
        with open(f'{self.task_id}_output_data.csv', 'w') as file:
