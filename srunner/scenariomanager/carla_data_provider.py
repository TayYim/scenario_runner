...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def get_car_follow_data(actor):
        ...
        Currently not considering the bounding box of the vehicle
        """
        ...
        max_distance = actor_velocity_vector.length() * 10.0  
        # Max detection range, the distance the actor travels in 10 seconds
        ...
        for other_actor in vehicles:
            if other_actor.id != actor.id:  # avoid comparing with itself
                ...
                distance = location_vector.length()  # Relative distance
                # Judge whether other_actor is within a certain range in front of actor according to the vector
                if actor_froward_vector.dot(other_actor_location - actor_location) > 0 \
                        and distance < max_distance:
                    front_vehicles_count += 1
                    # Calculate the projection of the relative velocity on the distance
                    ...
                    # Calculate the projection of the actor velocity on the distance
