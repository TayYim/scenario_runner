...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def get_car_follow_data(actor):
        """
        Get car follow data
        Include TTC, THW, relative velocity, relative distance
        Now considering the bounding box of the vehicle for more accurate measurements
        """

        # First check if this method has been called in this tick, _car_follow_data will be cleared every tick
        ...
        actor_right_vector = actor_transform.get_right_vector()
        actor_velocity_vector = CarlaDataProvider.get_velocity_vector(actor)
        actor_speed = actor_velocity_vector.length()
        max_distance = max(actor_speed * 10.0, 50.0)  # At least 50 meters detection range
        
        # Get actor's bounding box
        actor_bbox = actor.bounding_box
        actor_bbox_extent = actor_bbox.extent
        ...
        # Constants for determining relevance of other vehicles
        # Check vehicles within a wider angle to account for merging vehicles
        forward_angle_threshold = 80  # degrees - increased from typical 45 to catch merging vehicles
        
        for other_actor in vehicles:
            if other_actor.id != actor.id:  # avoid comparing with itself
                # Get other actor's bounding box
                other_bbox = other_actor.bounding_box
                other_bbox_extent = other_bbox.extent
                
                # Because other vehicles may not be generated by CDP (external traffic flow), the parameters of other vehicles are obtained using the carla interface
                ...
                # Use distance between centers initially for quick filtering
                distance_centers = location_vector.length()
                
                # Skip if vehicle is too far away (optimization)
                if distance_centers > max_distance:
                    continue
                    
                # Calculate angle to determine if vehicle is in front or merging
                forward_dot = actor_froward_vector.dot(location_vector.make_unit_vector())
                right_dot = actor_right_vector.dot(location_vector.make_unit_vector())
                angle = math.degrees(math.acos(max(-1.0, min(1.0, forward_dot))))
                
                # Check if the vehicle is within our forward field of view (including merging vehicles)
                if angle < forward_angle_threshold:
                    # Calculate velocity vector and distance considering bounding boxes
                    velocity_vector = actor_velocity_vector - other_actor_velocity_vector
                    
                    # Calculate approximate distance between vehicle surfaces instead of centers
                    # This is a simplified calculation - a more accurate one would use exact collision detection
                    distance_centers = location_vector.length()
                    
                    # Estimate the distance between surfaces by subtracting vehicle extents along the connecting line
                    direction = location_vector.make_unit_vector()
                    actor_extent_in_direction = (
                        abs(direction.x * actor_bbox_extent.x) + 
                        abs(direction.y * actor_bbox_extent.y) + 
                        abs(direction.z * actor_bbox_extent.z)
                    )
                    other_extent_in_direction = (
                        abs(direction.x * other_bbox_extent.x) + 
                        abs(direction.y * other_bbox_extent.y) + 
                        abs(direction.z * other_bbox_extent.z)
                    )
                    
                    # Actual distance between surfaces
                    distance = max(0.0, distance_centers - actor_extent_in_direction - other_extent_in_direction)
                    
                    front_vehicles_count += 1
                    
                    # Project velocities onto the connecting line for TTC calculation
                    rel_vel_projection = velocity_vector.dot(direction)
                    
                    # Calculate TTC - negative projection means vehicles are approaching
                    if rel_vel_projection < 0:
                        # Use max to avoid division by very small numbers which could lead to extremely large TTCs
                        current_ttc = distance / abs(rel_vel_projection)
                        ttc = min(ttc, current_ttc)
                    
                    # Calculate Time Headway (THW)
                    if actor_speed > 0.1:  # Only calculate if ego vehicle is moving
                        actor_vel_projection = actor_velocity_vector.dot(direction)
                        if actor_vel_projection > 0:
                            thw = min(thw, distance / actor_vel_projection)
                    
                    # Record relative velocity and relative distance
                    rel_velocity = max(rel_velocity, abs(rel_vel_projection))
